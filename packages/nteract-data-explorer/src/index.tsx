...
const findNotebookPanel = (widget: Widget): any => {
  ...
  while(currentWidget) {
    if (currentWidget.constructor && currentWidget.constructor.name === 'NotebookPanel') {
...
export class NteractDataExplorerWidget extends Widget implements IRenderMime.IRenderer {
  ...
  renderModel(model: IRenderMime.IMimeModel): Promise<void> {
    ...
    if (this._hasRendered) return Promise.resolve();
    ...
    const onMetadataChange = (data: any) => {
      ...
      if (notebookPanel) {
        notebookPanel.context.save().then(
          () => console.log('Save success.'),
          (reason: Error) => console.error('Save fails due to error: ', reason),
        );
      }
    };

    return new Promise<void>((resolve) => {
      // Use an interval timer to render Data Explorer whenever the current node is rendered in the screen.
      ...
      const timer = setInterval(() => {
        if (this.node.offsetWidth === 0 && this.node.offsetHeight === 0) return;
...
export const rendererFactory: IRenderMime.IRendererFactory = {
  safe: true,
  mimeTypes: [MIME_TYPE],
  createRenderer: options => new NteractDataExplorerWidget(options)
};
  ...
  fileTypes: [
    {
      ...
      // tdr is short for tabular-data-resource
